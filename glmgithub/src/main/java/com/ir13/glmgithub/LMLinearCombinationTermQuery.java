...
/**
 * Generalized Language model To be used with LMLinearCombinationTermQuery.
 * Defers the linear combination for later. Simply returns the tf/doclen The
 * collection stats are to be used later on.
 *
 * @lucene.experimental
 */
class GeneralizedLMSimilarity extends LMJelinekMercerSimilarity {
    ...
    public LMSimilarity.CollectionModel getCollectionModel() {
 ...
 * {@link org.apache.lucene.index.Term} occurs; NOTE: In order to take advantage
 * of this with the default scoring implementation
 * ({@link org.apache.lucene.search.similarities.DefaultSimilarity}), you must
 * override
 * {@link org.apache.lucene.search.similarities.DefaultSimilarity#scorePayload(int, int, int, BytesRef)},
 * which returns 1 by default.
 *
 * Payload scores are aggregated using a pluggable {@link PayloadFunction}.
 *
 * @see
 * org.apache.lucene.search.similarities.Similarity.SimScorer#computePayloadFactor(int,
 * int, int, BytesRef)
 *
 */
public class LMLinearCombinationTermQuery extends PayloadTermQuery {
    ...
    protected class LMLinearCombinationTermWeight extends PayloadTermQuery.PayloadTermWeight {
        ...
        protected class LMLinearCombinationTermSpanScorer extends PayloadTermQuery.PayloadTermWeight.PayloadTermSpanScorer {
            ...
            /**
             * The main tweak goes here, where instead of multiplying the
             * payload score, we add a linear combination of it and then take
             * log (1 + score_lm). Remember that the scorer in this case needs
             * to be changed to a new class which doesn't return the log as in
             * LMJelineckMercerSmoothing. The log of the linear combination has
             * to be taken here...
             *
